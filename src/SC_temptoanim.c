      ...
      while ( (filename = g_dir_read_name (directory)) ) { //filename should not be modified or freed
        ...
        if (g_pattern_match (png_pattern, strlen(filename), filename, NULL)) {
          ...
          if (glib_encoded_filename) {
            png[i] = g_strdup (glib_encoded_filename);
            g_free (glib_encoded_filename);
            i++;
          }
        }
...
static void make_mp4_from_temp (GtkWidget *widget)
{
  ...
  if (*p_anims_from_temp && *p_mp4) {
    char ff_make_movie_com[200], message[35];
    //construct ff_make_movie_com: ffmpeg -i temp.mkv -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" anim.mp4 
    strcpy (ff_make_movie_com, "/bin/sh -c '/usr/bin/ffmpeg -i temp.mkv ");
...
static void make_webm_from_pngs (GtkWidget *widget)
{
  ...
  if (!*p_anims_from_temp && *p_webm) {
    ...
    //construct ff_make_movie_com: ffmpeg -r fps -i ew-%3d.png -c:v libvpx -qmin 0 -qmax 50 -crf 5 -b:v 749k -y anim.webm
...
static void make_mp4_from_pngs (GtkWidget *widget)
{
  ...
  if (!*p_anims_from_temp && *p_mp4) {
    char ff_make_movie_com[200], char_fps[4], message[35];
    //construct ff_make_movie_com: ffmpeg -r fps -i ew-%3d.png -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -y anim.mp4 
    strcpy (ff_make_movie_com, "/bin/sh -c '/usr/bin/ffmpeg -r ");
    int *p_fps = P("p_fps");
    snprintf (char_fps, 4, "%d", *p_fps);
    strcat (ff_make_movie_com, char_fps);
...
static void show_spawn_dialog (GtkWidget *widget, char silentcast_dir[PATH_MAX], GPid pid, char *message) 
{
  if (strcmp (message, "")) { //don't show dialog if there's no message to show (strcmp will be 0 when they match)
    GtkWidget *spawn_dialog = gtk_application_window_new (gtk_window_get_application (GTK_WINDOW(widget)));
    gtk_window_set_transient_for (GTK_WINDOW(spawn_dialog), GTK_WINDOW(widget));
    gtk_window_set_title (GTK_WINDOW(spawn_dialog), silentcast_dir);
    gtk_window_set_modal (GTK_WINDOW(spawn_dialog), TRUE);
    g_child_watch_add (pid, (GChildWatchFunc) child_watch_cb, spawn_dialog);
    ...
    gtk_container_add (GTK_CONTAINER(spawn_dialog), dialog_grid);
#define DIALOG_ATCH(A,C,R,W) gtk_grid_attach (GTK_GRID(dialog_grid), A, C, R, W, 1) 
    DIALOG_ATCH(message_label, 0, 0, 4);
                                        DIALOG_ATCH(cancel_button, 4, 1, 1);

    gtk_widget_show_all (spawn_dialog);
  }
}
